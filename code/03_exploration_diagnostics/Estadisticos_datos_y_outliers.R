# ---------------------------------------------
# Estadisticos_datos_y_outliers.R (replication-ready)
# Boxplot comparativo de retornos + extremos por activo
#
# Input:  data_processed/<activo>_ret.csv
# Output:
#   - output/figures/diagnostics/plots/boxplot_retornos.png
#   - output/figures/diagnostics/plots/extremos_scatter.png
#   - replication_data/diagnostics/retornos_extremos.csv
# ---------------------------------------------
...
dir_input <- here("data_processed")

dir_fig_base <- here("output", "figures", "diagnostics")
dir_plots    <- file.path(dir_fig_base, "plots")
dir_diag_out <- here("replication_data", "diagnostics")

dir.create(dir_fig_base, showWarnings = FALSE, recursive = TRUE)
dir.create(dir_plots,    showWarnings = FALSE, recursive = TRUE)
dir.create(dir_diag_out, showWarnings = FALSE, recursive = TRUE)
...
for (a in activos) {
  path <- file.path(dir_input, paste0(a, "_ret.csv"))
...
if (nrow(retornos_stack) > 0) {
  ...
  ruta_boxplot <- file.path(dir_plots, "boxplot_retornos.png")
  png(ruta_boxplot, width = 1400, height = 800, res = 150)
  ...
  message("Boxplot guardado en: ", ruta_boxplot)
} else {
...
if (length(extremos_lista) > 0) {
  ...
  ruta_extremos_csv <- file.path(dir_diag_out, "retornos_extremos.csv")
  write.csv(tabla_extremos, file = ruta_extremos_csv, row.names = FALSE)
  message("Extremos guardados en: ", ruta_extremos_csv)
  
  ruta_scatter <- file.path(dir_plots, "extremos_scatter.png")
  png(ruta_scatter, width = 1600, height = 800, res = 150)
  par(mar = c(5, 4, 2, 1))
  cols <- as.factor(tabla_extremos$Activo)
  
  plot(
    tabla_extremos$Fecha, tabla_extremos$Retorno,
    pch = 16,
    xlab = "Fecha",
    ylab = "Retorno",
  ...
  message("Scatter de extremos guardado en: ", ruta_scatter)
} else {
  message("No se detectaron retornos extremos con umbral ±", umbral_extremo)
}

message("== Fin del script Estadisticos_datos_y_outliers.R ==")
