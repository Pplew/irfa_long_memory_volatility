# ==========================================================
# evaluacion_media_completa.R (versión replication package)
...
#   - replication_data/mean_models/ARFIMA_inicializado/resumen_arfima_inicializado.csv
#
# OUTPUT:
#   - replication_data/mean_models/ARFIMA_inicializado/evaluacion_media_completa.csv
#   - replication_data/mean_models/ARFIMA_inicializado/robustez_parametrica_d.csv
# ==========================================================
...
if (!file.exists(init_path)) {
  stop(
    "Falta el fichero de inicialización ARFIMA: ", init_path,
    "\nEjecuta antes el script que genera 'resumen_arfima_inicializado.csv'."
  )
}
...
# --------- datos disponibles (retornos procesados) ---------
archivos <- list.files(
  here("data_processed"),
  pattern = "_ret\\.csv$",
  full.names = TRUE
...
for (archivo in archivos){
  ...
  dt <- fread(archivo)
  if (!"retorno" %in% names(dt)) {
    cat("Falta 'retorno' en", archivo, "- salto.\n")
    next
  }
  y <- dt$retorno
  y <- y[is.finite(y)]
  n <- length(y)
  if (n < 300) {
    cat("Serie corta para", vis, "(n =", n, ") - salto.\n")
    next
  }
